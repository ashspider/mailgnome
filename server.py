...
def get_result(email, conn):
    ...
    try:
        ...
        result = {'code':5, 'message': 'Timeout'}
    except dns.resolver.NXDOMAIN as ex:
        result = {'code':4, 'message': 'Unknown'}
    except Exception as ex:
        result = {'code':0, 'message': 'Unknown'}
        print(ex.message)
    ...
    for mail_server in mail_servers:
        ...
        try:
            server = smtplib.SMTP(str(mail_server.exchange)[:-1],timeout=5) #@added timeout
        except Exception as ex:
            result = {'code':6, 'message': 'Unknown'} #Unable to connect to Mail Server
            print(result)
            continue
        try:
...
class root:
    ...
    def check_email(self, *args, **kwargs):
        #cherrypy.session.release_lock()# i added @todo
        parent_conn, child_conn = Pipe()
        print("Length",len(args))
        print len(args)==0
        if len(args)==0:
            result = json.dumps({'email':'None', 'code': 0, 'message': 'Invalid'})
        ...
        #if '@' not in email:
        if match is None:
            resp = json.dumps({'email':email,'code':0, 'message': 'Invalid'})
            child_conn.send(resp)
            child_conn.close()
